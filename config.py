...
# === 按键限速 / 抽稀重编曲 ===
# 把"最快按键间隔"限制到一个舒适上限：当原曲某段按键比上限更密时，
# 不延迟按键（会破坏节奏），而是按音乐重要性"抽稀"音符——保留旋律骨架
# （Skyline主旋律 + 强拍低音根音 + 长音/重音），丢掉夹在中间的经过音/装饰音。
# 保留下来的音符保持原始时间，尽量还原原歌曲走向。
PRESS_RATE_LIMIT_ENABLED = True   # 是否启用"最快按键间隔"限速（抽稀重编曲）
MIN_PRESS_INTERVAL_S = 0.3125     # 最快相邻按键间隔(秒)。0.3125≈3.2击/秒，
                                  #   对齐 青花瓷简单版-EOP教学曲(96BPM八分音符=312.5ms)的舒适节奏。
                                  #   调小=保留更多细节(更接近原曲)，调大=更简单更"教学化"。
MAX_PRESS_GROUP_NOTES = 2         # 每次按键(和弦)最多保留的音符数(最高音=旋律 + 最低音=低音根音)
